...
var useWebSocket = function() {
    ...
    websocket.onmessage = function(event) {
        try {
            ...
            pb.dispatchEvent('stream_message', JSON.parse(event.data));
        } catch (e) {
            pb.log('Couldn\'t parse message');
        }
    };
    websocket.onclose = function(event) {
        pb.log('WebSocket onclose');
    };
    websocket.onerror = function(event) {
        pb.log('WebSocket onerror');
        useFallback = false;
    };
};
        ...
        if (xhr.readyState == 3 && xhr.status == 200 && xhr.responseText.length > 0) {
            ...
            if (messages.length > 0) {
                ...
                for (var i = 0; i < numNewMessages; i++) {
                    ...
                    try {
                        pb.dispatchEvent('stream_message', JSON.parse(message));
                    } catch (e) {
                        pb.log('Couldn\'t parse message');
                    }
                }
