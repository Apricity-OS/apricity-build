...
utils.smsLength = function(v) { return v.replace(/([{}\[\]\\|\^~€])/g, "\\$1").replace(/\n/g, "--").length; };
...
utils.jsScroll = function(element) {
    element.onmousewheel = function(e) {
        e.preventDefault();
        element.scrollTop = ((e.wheelDelta / 2) * -1) + element.scrollTop;
    };
};

utils.targetImageUrl = function(target) {
    if (target) {
...
utils.targetDisplayName = function(target) {
...
utils.targetKeys = function(push) {
    var keys = [];
