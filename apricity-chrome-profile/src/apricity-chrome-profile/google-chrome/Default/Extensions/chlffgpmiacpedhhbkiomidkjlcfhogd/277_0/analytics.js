...
pb.track = function(event) {
    if (window.chrome && !chrome.runtime.getManifest().key) {
    ...
    event.client_type = window.chrome ? (pb.isOpera ? 'opera' : 'chrome') : (window.safari ? 'safari' : 'firefox');
...
var trackPer = function(time, event) {
    ...
    if (!localStorage[key] || parseInt(localStorage[key]) + time < now) {
        localStorage[key] = now;
        pb.track(event);
    } else if (window.chrome && !chrome.runtime.getManifest().key) {
        pb.log('Ignoring ' + event.name + ', last reported ' + (now - parseInt(localStorage[key])) + ' seconds ago');
    }
};
