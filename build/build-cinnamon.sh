...
make_packages() {
    setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "$(grep -h -v ^# ${script_path}/packages-cinnamon.{both,${arch}})" install
}
...
make_customize_airootfs() {
    cp -af ${script_path}/airootfs-cinnamon ${work_dir}/${arch}
    mv ${work_dir}/${arch}/airootfs-cinnamon ${work_dir}/${arch}/airootfs
